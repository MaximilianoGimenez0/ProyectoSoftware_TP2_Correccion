...
namespace WebApplication1.Controllers
{
    ...
    public class ProjectController : ControllerBase
    {
        ...
        [ProducesResponseType(typeof(ApiErrorResponse), 400)]
        public async Task<IActionResult> GetFiltered(string? title, string? status, string? applicant, string? approvalUser)
        {
            ...
            // Validar que status, applicant y approvalUser sean enteros positivos o null
            if (!IsNullOrPositiveInt(status, out int statusInt))
                return BadRequest(new ApiErrorResponse { message = "El filtro status debe ser un número entero positivo." });

            if (!IsNullOrPositiveInt(applicant, out int applicantInt))
                return BadRequest(new ApiErrorResponse { message = "El filtro applicant debe ser un número entero positivo." });

            if (!IsNullOrPositiveInt(approvalUser, out int approvalUserInt))
                return BadRequest(new ApiErrorResponse { message = "El filtro approvalUser debe ser un número entero positivo." });

            try
            {
                var results = await _projectProposalService.GetFiltered(title, statusInt == 0 ? (int?)null : statusInt,
                                                                      applicantInt == 0 ? (int?)null : applicantInt,
                                                                      approvalUserInt == 0 ? (int?)null : approvalUserInt);
                return Ok(results);
            }
            catch (BadRequestException ex)
            {
        ...
        [ProducesResponseType(typeof(ApiErrorResponse), 400)]
        public async Task<IActionResult> UpdateProjectInformation(Guid id, ProjectUpdateRequest request)
        {
            if (!ModelState.IsValid)
            {
                var firstError = ModelState.Values
            ...
            try
            {
                var updated = await _projectProposalService.UpdateProject(id, request);
                var results = await _projectProposalService.GetCompleteProjectGetById(id);
        ...
        private bool IsNullOrPositiveInt(string? value, out int parsed)
        {
            parsed = 0;
            if (string.IsNullOrEmpty(value)) return true; // null or empty = no filter

            if (!int.TryParse(value, out parsed)) return false; // no es número

            if (parsed < 1) return false; // no es positivo
